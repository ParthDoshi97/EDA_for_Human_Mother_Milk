...
# standardize the preprocessed data
#NSII_FAMES_HM_Normal<- predict(Autoscale,NSII_FAMES_HM_Normal)
#NSII_FAMES_HM_Normal <- NSII_FAMES_HM_Normal %>% 
#  replace(sapply(., is.infinite), 0) %>%
#  replace(sapply(.,is.na), 0)
  

skewness(NSII_FAMES_HM_Normal[2:30])

skimmed_FAMES_Scale <- skim(NSII_FAMES_HM_Normal)
skimmed_FAMES_Scale



# Converting it into long 
NSII_FAMES_HM_Normal.long <- gather(NSII_FAMES_HM_Normal[2:15])
NSII_FAMES_HM_Normal.long.subset <- gather(NSII_FAMES_HM_Normal[16:30])

# box plot after distribution
NSII_FAMES_HM_Normal.boxplot.A <- ggplot(NSII_FAMES_HM_Normal.long, aes(x = value, y = key, fill = key))+
  geom_boxplot(outlier.colour = "red", outlier.shape = 1) +
  labs(title = "Scaled FAMES Data Boxplot",)
print(NSII_FAMES_HM_Normal.boxplot.A)


# box plot after distribution
NSII_FAMES_HM_Normal.boxplot.B <- ggplot(NSII_FAMES_HM_Normal.long.subset, aes(x = value, y = key, fill = key))+
  geom_boxplot(outlier.colour = "red", outlier.shape = 1) +
  labs(title = "Scaled FAMES Data Boxplot",)
print(NSII_FAMES_HM_Normal.boxplot.B)
```

### Box plot of data distribution of each group (Post,Pre,PI,TI)

```{r}
NSII_FAMES_HM_Normal_A <- NSII_FAMES_HM_Normal[, c(1:15, 31)]
NSII_FAMES_HM_Normal_B <- cbind(NSII_FAMES_HM_Normal[1], NSII_FAMES_HM_Normal[16:31])
str(NSII_FAMES_HM_Normal_A)
...
boxplot <- function(data) {
  data %>%
    ...
    labs(title = "Boxplot of NSII_FAMES_HM_Normal",
         x = "NewClass",
         y = "Value")
}


# Create the boxplot for NSII_FAMES_HM_Normal_A
NSII_FAMES_HM_Normal_A_boxplot <- boxplot(NSII_FAMES_HM_Normal_A)
print(NSII_FAMES_HM_Normal_A_boxplot)

# Create the boxplot for NSII_FAMES_HM_Normal_B
NSII_FAMES_HM_Normal_B_boxplot <- boxplot(NSII_FAMES_HM_Normal_B)
print(NSII_FAMES_HM_Normal_B_boxplot)
...
p.mat <- cor_pmat(NSII_FAMES_HM_Normal[3:27], method = "pearson")

# Calculate correlation matrix using Pearson correlation
correlationMatrix <- cor(NSII_FAMES_HM_Normal[3:27], method = "pearson")
...
continuous.vars <- colnames(NSII_FAMES_HM_Normal[2:30])
categorical.var <- "NewClass"

NSII_FAMES_HM_Normal$NewClass <- factor(NSII_FAMES_HM_Normal$NewClass)
...
for (vars in continuous.vars) {
  ...
  var_test <- levene_test(formula, data = NSII_FAMES_HM_Normal)
  cat("Variable:", vars, "\n")
  print(var_test)
  cat("\n")
}
...
for (vars in continuous.vars) {
  formula <- as.formula(paste(vars, "~", categorical.var))
  kruskal_model <- kruskal.test(formula, data = NSII_FAMES_HM_Normal)
  cat("Variable:", vars, "\n")
  print(kruskal_model)
  cat("\n")
}
...
for (vars in continuous.vars) {
  formula <- as.formula(paste(vars, "~", categorical.var))
 pairwise_test <- dunn_test(formula, data = NSII_FAMES_HM_Normal)
  cat("Variable:", vars, "\n")
  print(pairwise_test)
  cat("\n")
}
...
for (var in continuous.vars) {
  ...
  regression.model <- lm(formula, data = NSII_FAMES_HM_Normal)
...
```{r}
NSII_FAMES_HM_PCA <- prcomp(NSII_FAMES_HM_Normal[3:27], scale. = TRUE, center = TRUE)

#Visualization of PCA
fviz_pca_ind(NSII_FAMES_HM_PCA,
             geom = "point",
             habillage = NSII_FAMES_HM_Normal$NewClass,
...
results <- rfe(NSII_FAMES_HM_Normal[,2:30], NSII_FAMES_HM_Normal[,31], sizes=c(1:27), rfeControl=control)
...
boruta_output <- Boruta(NewClass ~ ., data=na.omit(NSII_FAMES_HM_Normal), doTrace=2)  # perform Boruta search
